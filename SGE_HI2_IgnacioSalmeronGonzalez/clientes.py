...
def insert():
    ...
    if not nombre or not apellido or not direccion or not cp or not tlf:
        ...
        try:
            # Convertimos a enteros después de verificar campos completos
            cp = int(cp)
            tlf = int(tlf)
...
def clientes():
    ...
    titulo = ctk.CTkLabel(ventana, text="CLIENTES", font=("Eras Demi ITC", 30))
    ...
    ctk.CTkLabel(ventana, text="Apellido:", font=("Eras Demi ITC", 15)).grid(row=1, column=4, sticky="e", padx=10, pady=10)
    entryApe = ctk.CTkEntry(ventana)
    entryApe.grid(row=1, column=5, padx=15, pady=15)

    ctk.CTkLabel(ventana, text="Dirección:", font=("Eras Demi ITC", 15)).grid(row=2, column=0, sticky="e", padx=10, pady=10)
    entryDir = ctk.CTkEntry(ventana)
    entryDir.grid(row=2, column=1, padx=15, pady=15)

    ctk.CTkLabel(ventana, text="CP:", font=("Eras Demi ITC", 15)).grid(row=2, column=2, sticky="e", padx=10, pady=10)
    entryCp = ctk.CTkEntry(ventana)
    entryCp.grid(row=2, column=3, padx=15, pady=15)

    ctk.CTkLabel(ventana, text="Teléfono:", font=("Eras Demi ITC", 15)).grid(row=2, column=4, sticky="e", padx=10, pady=10)
    entryTlf = ctk.CTkEntry(ventana)
    entryTlf.grid(row=2, column=5, padx=15, pady=15)
    ...
    mostrarDatos(cursor, tablaClientes)
    tablaClientes.grid(row=4, column=1, padx=20, pady=20, rowspan=1, columnspan=5, sticky="nsew")
